...
- [x] **D. 골든 케이스 회귀셋** — ✅ 골격+실제 1건 완료 (2026-06-29). `tests/golden/*.json`(익명화·숫자만) + `test_golden_cases.py`(조례 stub + `_diagnose` skip_ai 결정론). 첫 케이스 = M드라이브 실 인허가 건축개요서에서 추출한 **오피스텔(일반상업)** — 엔진이 건폐율 68.04%·용적률 699.80%(법정 700 면도날 통과)를 정확 재현 확인. 식별정보(사업명·주소·발주처) 전부 제외, 수치·용도지역만 커밋. 스키마·익명화 규칙은 `golden/README.md`. 테스트 +1(전체 273). ※ 제거된 Step 8(케이스 매칭)과 다른 *정답 측정*. ⏳ 케이스 다양화(공동주택·완화·심의)는 사내 자료 추가 시 JSON만 더 넣으면 자동 수집.
