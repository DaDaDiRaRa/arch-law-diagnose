...
- [~] **사내 시설용도 매핑표** — **재료 추출 완료·전제 정정(2026-08-28)**. 산출물 `data/facility_use_mapping.csv`(19행), 재생성 `scripts/build_facility_use_mapping.py`. ⚠**"brief 샘플만 모으면 된다"는 전제가 틀렸다**: competition_comparison prod 21건+fixture 10건=30건은 **공모 8종**일 뿐이다(영등포 1건이 14회·대전 1건이 10회 재분석). 파일 수로 세면 표본이 4배 부풀어 보인다. 중복 제거 후 부지 9개·고유어휘 19종뿐이라 **이 데이터로는 전국용 매핑표를 못 만든다**(CLAUDE.md 원칙 #2 위반). **대안 확보**: 건축법 시행령 **[별표 1] 「용도별 건축물의 종류」**(개정 2025-10-01, 12쪽·10,924자)가 곧 매핑표다 — 법제처 DRF `별표서식PDF파일링크`로 취득 검증 완료(가드레일 예외 경로). 어휘 분류: ①이미 자동감지 4종 ②**별표1에 있음 2종 = 코드로 해결, 시니어 불필요**(보건소·학교) ③**별표1에 없음 11종 = 진짜 사람 판단**(주민편의시설·부대복리시설·부설주차장·연도형상가 등 공모지침 관용어) ④건축물 아님 2종(도로·공원). 즉 시니어 몫은 "전 어휘"가 아니라 **공모 관용어 ~11종**으로 줄었다. ⚠**곁가지 발견**: `_BUILDING_USES`가 **18종인데 별표1 용도군은 29개** — 운수·수련·자동차관련·방송통신·관광휴게·장례·야영장 등 **13개 용도군을 우리가 아예 모른다**(해당 용도 공모가 오면 무조건 후보 0). 별표1 파싱으로 같이 해결 가능. *(다음: 별표1 파서 → `_BUILDING_USES` 29종 확장 + 시설명 인덱스; 그 후 잔여 11종만 시니어)*
