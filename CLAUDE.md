...
- [ ] **B. 산정 근거(provenance) 블록** — 정량 4개 카드(건폐율·용적률·높이·주차)에 `{입력값·공식·결과·law_refs}` 묶음 필드 추가(기존 필드 유지). 검토 보고서 추적성↑.
- [ ] **D. 골든 케이스 회귀셋** — 실 인허가 통과 사례 주소→정답(건폐율·용적률·완화·심의) end-to-end 테스트(`tests/golden/`, respx 모킹/`diagnose_fast`로 결정론화). 코드 골격은 지금, 케이스 데이터는 사내 수집 대기. ※ 제거된 Step 8(케이스 매칭)과 다른 *정답 측정*.
