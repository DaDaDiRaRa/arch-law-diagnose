...
- [x] ~~**`feasibility_export` 부분 채택**~~ ✅ **2026-08-27 완료** — 실샘플 10건 전수 대조에서 정량치 100% 일치라 **전면 교체는 안 함**(파서 유지·폴백 영구 필요: 10건 중 4건은 블록 없음·1건 v1). `schema_version>=2`일 때만 **우리가 못 뽑는 3종을 가산**: `limits_determined_by`·`required_parking_count`+`parking_note`·사업규모(`scale`). `zone_use`는 미채택(그쪽 동점 오선택 버그 — 제보 후 그쪽이 커밋 `3a7d8e7`로 수정, 「2개 이상이면 판단 보류」로 우리 normalizer와 동일 동작). **실사용 오류 해소**: 심의로 상향된 용적률(영등포 460% vs 준공업 법정 400%)을 「초과·부족 60%p」로 오보하던 것 → 새 status `review_premised`(「심의 전제 · 법정 한도 대비 +60.0」, amber)로. ⚠**상한(max)에만 적용** — 주차 같은 법정 최소는 심의로 면제되지 않는다. 종합판단은 「패스 권장」이 아니라 「협상 필요」(심의 확정 여부는 우리가 검증 못 하므로 「참여 권장」으로도 단정 안 함). 경로: `brief_importer`(가산·`feasibility_export_used`) → `schemas.FeasibilityRequest.limits_determined_by`(Literal 검증) → `feasibility_engine._compute_gap`/`_compute_recommendation` → `GapChart` amber + `BriefImportPanel` 심의 배지·주차대수. 회귀 22건 추가(스텁 엔진 전 구간 통과 2건 포함), 테스트 318→340.
...
- [~] **사내 시설용도 매핑표** — **별표1 파서 완료(2026-08-28), 잔여 11종만 시니어 몫**. ① **표본 전제 정정**: prod 21건+fixture 10건=30건은 **공모 8종**일 뿐(영등포 14회·대전 10회 재분석) → brief 데이터만으론 전국용 매핑표 불가. ② **대안 채택**: 건축법 시행령 **[별표 1] 「용도별 건축물의 종류」**(개정 2025-10-01)를 `config/building_use_seed.json`으로 **전사**(29용도군·시설명 208종). 가드레일 조건대로 build/verify 분리 — `scripts/build_building_use_seed.py`(전사) · `scripts/verify_building_use_seed.py`(현행 재다운로드 후 **전수 대조**, 파서는 `services/building_use_table.py` 하나를 공유). ③ **감지 2단계화**: 용도군명 직접매칭 + 별표1 시설명 중 **단일·무조건 125종만** 자동채움(`safe_names`), **조건부·모호 83종은 힌트만**(`_facility_use_hints` — 보건소는 1천㎡ 미만 제1종근생/초과 업무시설이라 이름만으론 못 정함). 시설명은 **정확일치만**(부분매칭 시 '세대창고'→창고시설 오판), **부설·부대·부속은 주용도 아님**으로 제외. ④ **용도군 18→29 확장**: 운수·수련·자동차관련·방송통신·위험물·동식물·자원순환·교정·발전·묘지·관광휴게·장례·야영장 **13종을 아예 고를 수 없었다**. 프론트 드롭다운은 4개 파일에 복사돼 있던 걸 `frontend/src/constants/buildingUses.js` 단일 소스로 통합(백엔드 어휘와 철자 일치를 `test_frontend_dropdown_covers_backend_vocab`이 강제 — 불일치 시 prefill이 **조용히** 무시된다). ⑤ 실측: 현 코퍼스 자동채움 14→13(공동주택+학교 혼합 부지가 후보 2개로 정직하게 노출됨). **이득은 자동채움률이 아니라 표현 불가능하던 13개 용도군 커버리지**. 남은 것 → 산출물 `data/facility_use_mapping.csv`의 **B등급 11종**(주민편의시설·부대복리시설·부설주차장·구청·구의회·연도형상가 등 법에 없는 공모 관용어)만 시니어 기입. *(사용자 처리)*
