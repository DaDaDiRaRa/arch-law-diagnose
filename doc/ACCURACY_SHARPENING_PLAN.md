...
### D. 골든 케이스 회귀셋 (계산 엔진 #1 정확도 자산) — ✅ 골격+실제 1건 (2026-06-29)
...
> 첫 케이스: M드라이브 실 인허가 건축개요서 → **오피스텔(일반상업)**, 식별정보 전부 제외·수치만.
> 엔진이 건폐율 68.04%·**용적률 699.80%(법정 700 면도날 통과)** 정확 재현 확인. 테스트 +1(전체 273).
> ⏳ 다양화(공동주택·완화·심의)는 golden/ 에 JSON만 추가하면 자동 수집.
