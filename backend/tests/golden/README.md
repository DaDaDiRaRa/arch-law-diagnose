...
## 현재 커버리지 (12건, 2026-06-29)

용도지역 7종 × 다양한 유형. 다수가 법정/허용/상한 **면도날 통과**를 정확 재현.
...
| knowledge_industry_semi_industrial_01 | 준공업 | 지식산업센터 | 59.89 / 60 | 398.90 / 400 |
| lodging_general_commercial_01 | 일반상업 | 숙박시설 | 69.36 / 70 | 498.58 / 500 |
| mixed_use_district_plan_01 | 지구단위(업무복합용지) | 주상복합 | 59.95 / 60 | 269.57 / 270 |

**남은 용도지역**(미수집): 제1종일반주거·전용주거·근린상업·유통상업·일반/전용공업·보전/생산/자연녹지.
**남은 시설**(미수집): 근린생활·의료·노유자·물류·공장·단독/다세대.
> 참고: 건폐율·용적률 정답값은 *용도지역*이 좌우하고 building_use(시설)는 거의 무관(주차만 영향). 다양성 우선순위는 용도지역.

**고가치 경로 테스트 4종** (`test_path_*`, 합성 — 실 통과 케이스가 못 때리는 경로): ✅ 전부 통과(엔진 정확).
> ① 실패/RED(한도 초과 → pass=False·signal RED) ② 완화(공개공지 25.18%+녹색최우수 → `far_relief` 전체 캡 1.15배=460%) ③ 주차 부족(pass=False·deficit) ④ 비주입(applied_ordinance 없이 엔진이 `zone_limits.json` 시행령 한도 60/250 자가 결정).
> → 통과 = 해당 경로가 *현재 올바름*을 확인 + 회귀 고정. 여기서 실패가 나는 날이 곧 정확도 버그 발견.
